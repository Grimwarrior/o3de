...
**These replace an earlier 0.10 average / 0.17 marginal, which are now about 4.5x too high.** They were measured before AZSLc
started parsing in SLL mode, which cut the parse -- two thirds of an AZSLc run -- by 57%. Anything
...
Those absolute figures are long out of date -- the same job is now 0.530 s with azslc at about 234 ms, after the AZSLc and DXC
work that landed separately. The ratio is what this section is about and it still holds: enabling a second backend runs the whole
MCPP to azslc to DXC chain a second time.
